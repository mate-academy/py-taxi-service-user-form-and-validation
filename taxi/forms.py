...
class DriverLicenseUpdateForm(UserCreationForm):
    ...
    def clean_license_number(self):
        if not len(self.cleaned_data["license_number"]) == 8:
            raise ValidationError("license number must consist only 8 charactrers")
        if (not self.cleaned_data["license_number"][:3] != self.cleaned_data["license_number"][:3].upper()
